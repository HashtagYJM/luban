...
def _default_text(plat: str) -> str:
    return (
        ...
        "# warn_tokens = 150000\n"
        "# Clear stale tool results server-side. NOT a general token saver: what it\n"
        "# clears sits inside the cached prefix, so every clear throws that cache away\n"
        "# and the next call re-writes it. The saving is a trickle per call; the cache\n"
        "# write is a lump, and it grows with the size of your conversation — so the\n"
