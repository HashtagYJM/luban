...
REFLECT_PROMPT = (
    ...
    "1. SURVEY — read the whole store below, then read the one-line index of every fact "
    "as a LIST and ask, pair by pair, which of them say the same thing in different "
    "words. Do this before you look at the overlap scores. Those scores compare shared "
    "WORDS, so they find only the duplicates that happen to be worded alike; a rule "
    "stated twice in different language scores low and is still one rule stated twice. "
    "The listing is a floor, not the set of pairs to consider, and 'nothing above the "
    "threshold' is not a finding.\n"
    "2. MERGE — one fact per idea, judged by MEANING and not by wording. Where two facts "
    "say the same thing, write the better combined fact with remember (reuse the clearer "
    "slug) and forget the other. Where several facts each restate one shared point, that "
    "point belongs in one fact.\n"
    ...
    "it before you act.\n"
    "   USER.md IS NOT A DUMPING GROUND. It is sent on every turn, so every line costs "
    "forever, and unlike the fact store it does not degrade gracefully: past its cap the "
    "end of the file is simply cut. So graduation is a TRADE, not an append. Before "
    "promoting anything, check the always-on budget shown below, and:\n"
    "   - apply a high bar: would you want this in front of the model on EVERY turn for "
    "the next year? If not, leave it as a fact.\n"
    "   - write ONE tight line, not a paragraph. Fold it into an existing bullet if one "
    "fits rather than adding a new one.\n"
    "   - pay for it: if USER.md is near or over its cap, tighten or drop weaker lines in "
    "the SAME edit so the file ends up under budget. Promoting nothing is a fine outcome.\n"
    "6. TIGHTEN — if USER.md is over budget even after that, edit it down regardless of "
    "whether you graduated anything. An over-budget USER.md is silently losing its tail "
    "right now.\n"
